...
- 项目批准纳入版本控制的正式题库、审查证据、模型评测证据和当前 SQLite 基线统一放在 `var/` 并提交 Git；未在模块 SDD 中明确批准的其他运行时数据仍不得提交。
...
- `var/` 获准基线之外的 SQLite、运行时数据和日志；`var/turtle_soup.sqlite3` 由显式否定规则纳入版本控制。
- 常见 IDE 与操作系统临时文件。

`.dockerignore` 必须阻止 Git 元数据、密钥、本地依赖、缓存、测试覆盖率、构建输出、`.agently` 运行时目录和整个 `var/` 进入未来容器构建上下文。`var/` 进入 Git 不表示其内容可以进入镜像。
