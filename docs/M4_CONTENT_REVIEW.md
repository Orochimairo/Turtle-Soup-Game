...
- 当前 M4 验收状态如下：
  ...
  - M4 已通过提交 `60bd609` 完成冻结并上传；上传验收时本地 `HEAD` 与 `origin/main` 一致。
  - 当前可以开始 M5 只读规划；M5 规格批准前不得实施 M5。
- `REJECTED` 候选不进入本批次后续流程；重新提出时必须使用新的临时候选引用。

本文档属于可上传的脱敏审查索引，只保存候选引用、候选名称、非剧透内容分类、状态、日期、证据引用和结论。完整题面、题底和核心事实保存在受版本控制的 `var/catalog/catalog.v1.json`，完整内容审查材料保存在受版本控制的 `var/catalog/private_review/`；不得复制到本文档。获准审核者必须直接检查对应材料；本文档中的摘要、结论或文件哈希不能替代该复核。
...
1. 重新确认 M4 冻结提交 `60bd609` 已上传，并在规划开始前运行上一冻结基线。
2. 只读规划 M5 的模型语义判定规格；规划必须继承 M1-M4 冻结契约、正式题库内容边界和人工盲测取消后的证据限制。
3. M5 只读规划完成并经用户批准后，才允许生成实施提示词；规格批准本身不构成编码授权。

当前不得修改既有审查或批准历史、改变正式题目内容、重复导入题库或直接实施 M5。M4 不再要求或追加 `BLIND_TESTED`。

## 9. 内容审查批次结果（2026-08-26）

依据 `docs/M4_PUZZLE_CATALOG.md` §6.2 对 30 个当时的预冻结题库引用完成逐题内容质量审查；逐题记录与详细依据保存在受版本控制的 `var/catalog/private_review/`（证据引用前缀 `PRIVATE-CONTENT-REVIEW-V1`）。审查发生时这些 `ts-NNNN` 只用于映射候选和审查证据；其后已因对应候选完成 `APPROVED` 转换而取得正式 ID 身份。
...
| SQLite 基线 | 受版本控制的 `var/turtle_soup.sqlite3` |
| M4 冻结提交 | `60bd609` |
| 上传确认 | 已完成；验收时本地 `HEAD` 与 `origin/main` 一致 |
| 最终冻结状态 | 已完成，可以开始 M5 只读规划；不得直接实施 M5 |

本摘要只记录脱敏事实，不包含题面、题底或核心事实。完整题库固定保存在受版本控制、但仍被 Docker 排除的 `var/catalog/catalog.v1.json`；本摘要不能替代对该文件、审查材料、Git 跟踪状态、Docker 排除边界和正式 ID 映射的独立内容冻结复核。
...
- 当前验收数据库基线保存在受版本控制的 `var/turtle_soup.sqlite3`；数据库仍可能随后续模块变化，因此本文档不把其 SHA-256冻结为长期契约，每次提交必须通过 Git 差异和真实数据库检查复核。
