...
M4 只负责建立经过内容审查和用户逐题批准的本地私有海龟汤题库，并提供使用 M3 冻结 Repository 将用户明确指定的本地题库显式导入已初始化 SQLite 数据库的能力。完整题面、题底、核心事实和私有内容审查材料不得进入 Git、GitHub 或 Docker 构建上下文；脱敏审查证据按本规格进入版本控制。
...
本规格与脱敏后的 M4 内容审查记录属于 Git 冻结证据，应与获准实现一起提交和上传。真实完整题面、题底、核心事实和私有内容审查材料仍不得进入 Git。

## 2. 所有者与依赖边界

M4 包含三个所有者边界：

- 题库策展负责在受版本控制的脱敏审查文档中完成候选登记、内容审查结论和批准记录，并在被忽略的 `var/catalog/private_review/` 中保存完整内容工作稿与私有内容审查材料。
...
- 来源元数据属于本地私有 M4 题库资源，不增加到 `Puzzle`，也不写入 SQLite。
...
- `docs/M4_CONTENT_REVIEW.md`：保存候选引用、来源记录、追加式门禁历史、内容审查摘要、预冻结题库引用或正式 ID、逐题批准记录和私有题库哈希；不得保存完整题面、题底、核心事实或私有内容审查原文。
...
- `var/catalog/private_review/`：保存门禁复核所需但不得公开的正式入库前完整内容工作稿和私有内容审查材料；内部文件名和格式不属于公共契约。

隐私与版本控制契约：

- 仓库根目录 `.gitignore` 必须继续以 `var/` 排除整个私有目录；不得使用 `git add -f` 绕过。
- 仓库根目录 `.dockerignore` 必须继续以 `var` 排除整个私有目录；Docker 构建上下文不得包含正式题库或私有内容审查材料。
...
- 私有题库和私有审查材料不得复制到 `backend/`、`frontend/`、`docs/`、测试夹具、日志、`.agently/`、缓存、构建产物或其他 Git 可见路径。
- 不创建或提交真实题库示例、删减副本、快照、补丁、备份或导出文件；私有题库 SHA-256 可以记录在脱敏的 `docs/M4_CONTENT_REVIEW.md` 中。
- `catalog.v1.json` 缺失时加载和导入必须明确失败。门禁所需的私有证据或审查记录缺失时，对应候选不得转换状态或冻结；Git 中的代码和测试不得生成、下载或补齐任何私有内容。
...
缺少字段、额外字段、重复键、空白字符串或未知 `source_kind` 均失败。来源对象只作为本地私有内容和审查记录，不传给 `PuzzleRepository`，也不得原样复制到版本控制文件；`docs/M4_CONTENT_REVIEW.md` 只能记录必要的脱敏引用和结论。
...
- 测试收集和模块导入不要求 `var/catalog/catalog.v1.json` 存在，也不读取私有题库。
...
- 本地私有 `var/catalog/catalog.v1.json` 与受版本控制的脱敏 `docs/M4_CONTENT_REVIEW.md` 的正式 ID 集合完全一致。
- 从 `backend/src` 使用固定正式 CLI 命令成功解析并导入本地私有题库，且通过 M3 公共 Repository 核对全部正式 ID、状态和字段往返。
- `docs/M4_CONTENT_REVIEW.md` 记录题库文件 SHA-256、题目总数、启用数、验收日期和用户逐题批准；只记录脱敏证据引用，不记录完整题目内容或私有内容审查原文。
- `git check-ignore -v var/catalog/catalog.v1.json var/catalog/private_review/` 证明正式题库和私有审查目录由仓库 ignore 契约排除；`AGENTS.md` 和 `docs/` Markdown 不得被 ignore。
- Docker 构建上下文检查证明 `var/` 未进入上下文；不得通过复制、构建参数或其他路径重新引入。
- 对 Git 已跟踪文件、全部未跟踪候选和暂存内容执行题库泄露扫描，确认不存在完整题面、题底、核心事实或私有内容审查原文。
...
4. 将预冻结工作稿整理为只包含获准正式题目的 `var/catalog/catalog.v1.json`，确认所有预留引用均已取得正式 ID 身份，并验证 Git 与 Docker 隐私边界；在此之前不得加载、导入或冻结该工作稿。
5. 先编写 M4 契约测试并形成有效红灯。
6. 实现满足规格的最小 catalog 模块和 CLI。
7. 运行 M4 测试、M1-M3 回归和全量检查。
8. 独立复核受版本控制的规格与审查文档、代码、全部未跟踪文件、Git 差异、被忽略私有题库的明确存在与 ignore 证据、真实 SQLite 和人工证据；报告中不得泄露真实题目内容。
9. 用户分别确认 Git 规格与实施冻结和本地私有内容冻结；`AGENTS.md`、`docs/` 规格与脱敏审查文档、源码、合成测试和必要配置可以获准提交并上传，正式题库不得提交。

内容策展和编码可以由不同实施者完成。编码实施者不得自行搜索、生成、读取或替换真实题目，也不得把私有内容复制进测试；只根据格式契约和纯合成数据实现代码。私有题库导入的人工验收由获准访问 `var/catalog/` 的验收者执行。
...
git status --short --ignored -- var/catalog
git check-ignore -v var/catalog/catalog.v1.json var/catalog/private_review/
...
还必须单独读取全部未跟踪文件、受版本控制的规则与规格文档，以及明确路径下被忽略的正式题库和私有审查材料；对版本控制候选文件执行密钥和题库泄露扫描，并确认完整题面、题底和核心事实只存在于被忽略的 `var/catalog/catalog.v1.json`、`var/catalog/private_review/` 和导入后的本地 SQLite 中，没有副本进入 Git 候选、Docker 构建上下文、前端、日志、`.agently/`、测试夹具、缓存或构建产物。被忽略文件不出现在普通 `git status` 或 `git ls-files --others` 中，复核必须使用明确路径检查私有文件存在性、证据映射、格式、哈希、ignore 来源和真实导入结果。人工验收逐项报告结论和限制，不输出完整题目、题底、核心事实或来源隐私。
...
- 本地私有 JSON 版本 1 的结构、严格校验和来源元数据契约，但不包含真实内容。
- M4 公共本地加载与显式导入入口及失败边界。
- 真实 SQLite 导入、幂等、预检和非批量原子性语义。
- 只使用合成内容的 M4 自动化契约测试。
- Git、GitHub、Docker、测试和日志不得包含正式题库完整内容或私有内容审查原文的隐私边界。

不得进入 Git 或上传的本地私有内容冻结包括：

- `var/catalog/catalog.v1.json` 的正式题库内容。
- `var/catalog/private_review/` 中支持内容审查结论的完整工作稿与记录。
- 使用固定 CLI 命令完成的本地真实 SQLite 导入与逐字段核对证据。

私有内容冻结不依赖 Git 提交重建。私有题库或对应私有审查材料遗失、题库哈希变化、证据映射失效，或者与 `docs/M4_CONTENT_REVIEW.md` 的脱敏冻结记录不匹配时视为未冻结，必须重新验收；不得从 Git、测试夹具、网络或模型生成结果恢复私有内容。

后续模块不得修改正式题目的核心内容、复用 ID、放宽内容门禁或改写 M4 契约测试。需要新增题目时必须遵循同一获取和审查流程；需要改变冻结格式或公共入口时必须停止并取得用户批准。

只有本地私有内容冻结已经由用户确认、且 Git 规格与实施冻结已经提交并上传后，才能开始 M5。M5 可以通过本地 SQLite 使用获准题目进行真实模型语义评测，但不得直接读取私有题库文件，也不得反向修改题库以迎合模型输出。
